...
class RNN(nn.Module):
	def __init__(self, vocab_size, embedding_dim, rnn_units=64, num_layers=2, dropout=0.2, activation=None, batch_normalization= False, bidirectional=False):
		...
		self.dense2=nn.Linear(rnn_units,3)
	def forward(self, x):
		x=self.embedding(x)
		x, _ = self.rnn(x)
		x=x[:,-1,:]
